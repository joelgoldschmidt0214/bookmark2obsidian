...
class MarkdownGenerator:
    ...
    def generate_obsidian_markdown(self, page_data: Dict, bookmark: Bookmark) -> str:
        ...
        try:
            ...
            # 記事本文をMarkdown形式に変換
            markdown_content = self._format_content_for_obsidian(
            ...
            full_markdown = self._build_complete_markdown(
                yaml_frontmatter, markdown_content, obsidian_tags, page_data, bookmark
            )

            logger.debug(
                f"📝 Markdown生成成功: {bookmark.title} (文字数: {len(full_markdown)})"
            )
    ...
    def generate_file_path(
        self, bookmark: Bookmark, base_path: Path, avoid_duplicates: bool = True
    ) -> Path:
        """
        重複回避機能を強化したファイルパス生成
        ...
        try:
            ...
            if avoid_duplicates:
                final_filename = self._generate_unique_filename(
                    directory_path, base_filename, ".md"
                )
            else:
    ...
    def _build_complete_markdown(
        ...
        if bookmark.add_date:
            sections.append(
                f"**ブックマーク日時:** {bookmark.add_date.strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
    ...
    def _generate_unique_filename(
        ...
        while True:
            ...
            if counter > 1000:
                ...
                logger.warning(
                    f"⚠️ 重複回避上限到達、タイムスタンプ使用: {fallback_filename}"
                )
