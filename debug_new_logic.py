...
def analyze_new_logic(html_file_path):
    ...
    print(f"  - 重複による余分なブックマーク数: {sum(duplicates.values()) - len(duplicates)}")
    
    if duplicates:
        print(f"\n🔍 重複URLサンプル (上位10個):")
        for i, (url, count) in enumerate(sorted(duplicates.items(), key=lambda x: x[1], reverse=True)[:10]):
            print(f"  {i+1:2d}. {count}回: {url[:80]}...")
    
    # フォルダ別統計
    folder_stats = Counter()
    for bookmark in bookmarks:
        folder_path = '/'.join(bookmark.folder_path) if bookmark.folder_path else '(ルート)'
