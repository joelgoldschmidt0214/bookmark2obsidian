...
def analyze_bookmark_file(html_file_path):
    ...
    print(f"  - 全Aタグ数: {len(all_a_tags)}")
    print(f"  - 解析されたブックマーク数: {len(bookmarks)}")
    print(f"  - 除外率: {((len(all_a_tags) - len(bookmarks)) / len(all_a_tags) * 100):.1f}%")
            ...
            if a_tag:
                ...
                if url and title:
                    ...
                    if parser._should_exclude_bookmark(temp_bookmark):
                        ...
                        if level == 0 and excluded_count <= 5:  # ルートレベルの最初の5個だけ表示
                            print(f"{indent}  ❌ 除外 {excluded_count}: {title[:50]}... ({reason})")
                    else:
                        bookmark_count += 1
                        if level == 0 and bookmark_count <= 5:  # ルートレベルの最初の5個だけ表示
                            print(f"{indent}  ✅ 有効 {bookmark_count}: {title[:50]}...")
    
    print(f"{indent}📊 レベル {level} 集計: フォルダ={folder_count}, 有効ブックマーク={bookmark_count}, 除外={excluded_count}")
