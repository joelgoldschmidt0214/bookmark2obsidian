    ...
    for i, dt in enumerate(direct_dt_elements):
        ...
        if next_sibling and next_sibling.name == 'dd':
            ...
            if h3:
                ...
                if nested_dl:
                    analyze_dl_structure_detailed(nested_dl, parser, level + 1, max_level)
        else:
            # ブックマーク候補
            a_tag = dt.find('a')
            if a_tag:
                ...
                if url and title:
                    ...
                    if parser._should_exclude_bookmark(temp_bookmark):
                        ...
                        if excluded_count <= 3:  # 最初の3個だけ表示
                            ...
                            print(f"{indent}  ❌ 除外 {excluded_count}: {title[:30]}... ({reason})")
                    else:
                        bookmark_count += 1
                        if bookmark_count <= 3:  # 最初の3個だけ表示
                            print(f"{indent}  ✅ 有効 {bookmark_count}: {title[:30]}...")
...
def compare_processed_vs_unprocessed(soup, parser):
    ...
    print(f"  - 構造解析で処理されないAタグ数: {len(all_a_tags) - len(processed_a_tags)}")
