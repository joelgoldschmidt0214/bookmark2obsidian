...
class DebugBookmarkParser(BookmarkParser):
    ...
    def _parse_dl_element(self, dl_element, current_path):
        ...
        indent = "  " * level
        print(f"{indent}📁 レベル {level} 処理開始: パス={'/'.join(current_path) if current_path else '(ルート)'}")
        ...
        for i, dt in enumerate(direct_dt_elements):
            # 最初の数個だけ詳細表示
            show_detail = (level == 0 and i < 5) or (level == 1 and i < 3) or (level >= 2 and i < 2)
            ...
            if next_sibling and next_sibling.name == 'dd':
                ...
                if h3:
                    ...
                    if show_detail:
                        print(f"{indent}  📂 フォルダ {folder_count_this_level}: {folder_name}")
                    ...
                    if nested_dl:
                        ...
                        if show_detail:
                            print(f"{indent}    → {len(nested_bookmarks)}個のブックマークを取得")
                    elif show_detail:
                ...
                if a_tag:
                    ...
                    if url and title:
                        ...
                        if bookmark and not self._should_exclude_bookmark(bookmark):
                            ...
                            if show_detail:
                                print(f"{indent}  ✅ ブックマーク {bookmark_count_this_level}: {title[:30]}...")
                        else:
                            excluded_count_this_level += 1
                            if show_detail:
                                reason = self._get_exclusion_reason(bookmark) if bookmark else "抽出失敗"
                                print(f"{indent}  ❌ 除外 {excluded_count_this_level}: {title[:30]}... ({reason})")
        ...
        print(f"{indent}📊 レベル {level} 完了: フォルダ={folder_count_this_level}, ブックマーク={bookmark_count_this_level}, 除外={excluded_count_this_level}")
    ...
    def print_statistics(self):
        ...
        for level in sorted(self.level_stats.keys()):
            ...
            print(f"  レベル {level}: フォルダ={stats['folders']}, ブックマーク={stats['bookmarks']}, 除外={stats['excluded']}")
