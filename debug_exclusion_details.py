...
def detailed_exclusion_analysis(html_file_path):
    ...
    print(f"📊 除外率: {((len(all_a_tags) - valid_count) / len(all_a_tags) * 100):.1f}%")
    ...
    print(f"ドメインルートとして除外されたサンプル (最初の20件):")
    ...
    print(f"無効URLとして除外されたサンプル (最初の10件):")
...
def analyze_domain_root_logic():
    ...
    for url in test_urls:
        ...
        print(f"      パス: '{path}', クエリ: '{parsed.query}', フラグメント: '{parsed.fragment}'")
