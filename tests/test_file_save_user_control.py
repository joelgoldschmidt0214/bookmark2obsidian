...
class TestLocalDirectoryManagerEnhanced:
    ...
    def test_create_directory_structure_with_special_characters(self, temp_directory):
        ...
        # 特殊文字がサニタイズされることを確認
        expected_path = temp_directory / "フォルダ_スラッシュ" / "フォルダ_コロン" / "フォルダ"
        assert result_path == expected_path
        assert result_path.exists()
    ...
    def test_sanitize_folder_name(self, temp_directory):
        ...
        assert manager._sanitize_folder_name("folder/with\\slash") == "folder_with_slash"
        assert manager._sanitize_folder_name("folder:with:colon") == "folder_with_colon"
        assert manager._sanitize_folder_name("folder<>with<>brackets") == "folder_with_brackets"
        
        # 連続するアンダースコアの処理
        assert manager._sanitize_folder_name("folder___multiple___underscores") == "folder_multiple_underscores"
    ...
    def test_validate_file_save_operation_long_filename(self, temp_directory):
        ...
        if len(long_filename) > 255:
            ...
            assert any("ファイル名が長すぎます" in error for error in validation['errors'])
        else:
            # 255文字以下の場合は有効
            assert validation['valid'] == True
    ...
    def test_validate_file_save_operation_long_path(self, temp_directory):
        ...
        if len(str(file_path)) > 260:
            assert any("パスが長すぎる" in warning for warning in validation['warnings'])
    ...
    def test_validate_file_save_operation_permission_error(self, temp_directory):
        ...
        try:
            ...
            if not validation['valid']:
                assert any("書き込み権限がありません" in error for error in validation['errors'])
            else:
...
class TestMarkdownGeneratorEnhanced:
    ...
    def test_generate_file_path_with_duplicate_avoidance(self, markdown_generator, sample_bookmark, temp_directory):
        """重複回避機能付きファイルパス生成テスト"""
        # 既存ファイルを作成
        ...
        # 重複回避有効でファイルパス生成
        file_path = markdown_generator.generate_file_path(sample_bookmark, temp_directory, avoid_duplicates=True)
        
        # 重複回避されたファイル名になることを確認
        assert file_path.name == "テスト記事_001.md"
        assert file_path.parent == existing_dir
    
    def test_generate_file_path_without_duplicate_avoidance(self, markdown_generator, sample_bookmark, temp_directory):
        """重複回避機能なしファイルパス生成テスト"""
        # 既存ファイルを作成
        existing_dir = temp_directory / "技術" / "Python"
        existing_dir.mkdir(parents=True)
        ...
        # 重複回避無効でファイルパス生成
        file_path = markdown_generator.generate_file_path(sample_bookmark, temp_directory, avoid_duplicates=False)
    ...
    def test_generate_unique_filename_basic(self, markdown_generator, temp_directory):
        """基本的なユニークファイル名生成テスト"""
        # 既存ファイルなし
        filename = markdown_generator._generate_unique_filename(temp_directory, "test", ".md")
        assert filename == "test.md"
    
    def test_generate_unique_filename_with_duplicates(self, markdown_generator, temp_directory):
    ...
    def test_generate_unique_filename_fallback(self, markdown_generator, temp_directory):
        """フォールバック機能のテスト"""
    ...
    def test_generate_file_path_special_characters_in_title(self, markdown_generator, temp_directory):
    ...
    def test_generate_file_path_error_handling(self, markdown_generator, temp_directory):
        """エラーハンドリングのテスト"""
        # 無効なブックマークオブジェクト
        ...
        file_path = markdown_generator.generate_file_path(invalid_bookmark, temp_directory)
    ...
    def test_generate_file_path_deep_hierarchy(self, markdown_generator, temp_directory):
        """深い階層でのファイルパス生成テスト"""
        bookmark = Bookmark(
            title="深い記事",
        ...
        expected_path = temp_directory / "レベル1" / "レベル2" / "レベル3" / "レベル4" / "レベル5" / "深い記事.md"
        assert file_path == expected_path
    
    def test_generate_file_path_no_folder_path(self, markdown_generator, temp_directory):
    ...
    def test_generate_file_path_none_folder_path(self, markdown_generator, temp_directory):
