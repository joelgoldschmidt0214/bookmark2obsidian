...
from app import (
    BookmarkParser, LocalDirectoryManager, WebScraper, 
    MarkdownGenerator, Bookmark, validate_bookmarks_file,
...
class TestIntegration:
    ...
    def test_bookmarks_html(self):
        ...
        return (project_root / "test_data" / "test_bookmarks.html").read_text(encoding='utf-8')
    
    @pytest.fixture
    def empty_bookmarks_html(self):
        """空のbookmarks.htmlの内容を提供"""
        return (project_root / "test_data" / "empty_bookmarks.html").read_text(encoding='utf-8')
    ...
    def test_full_workflow_with_test_data(self, temp_directory, test_bookmarks_html):
        ...
        normal_bookmark = next(b for b in bookmarks if b.title == "Python Documentation")
    ...
    def test_directory_validation_edge_cases(self):
        ...
        assert not is_valid
        assert "入力してください" in message or "指定されていません" in message or "空" in message
...
class TestEdgeCases:
    ...
    def test_invalid_url_handling(self):
        ...
        for url in invalid_urls:
            try:
                ...
                # 例外が発生することも許容される
                assert isinstance(e, (requests.exceptions.RequestException, ValueError, Exception))
    ...
    def test_network_error_handling(self):
        ...
        # 存在しないドメイン
        try:
            result = scraper.fetch_page_content("https://this-domain-absolutely-does-not-exist-12345.com")
            assert result is None
    ...
    def test_empty_folder_path_handling(self, temp_directory):
        ...
        bookmarks_with_empty_paths = [
            Bookmark(title="Root Bookmark 1", url="https://example.com", folder_path=[]),
            Bookmark(title="Root Bookmark 2", url="https://example.com", folder_path=None),
        ]
...
class TestErrorRecovery:
    ...
    def test_graceful_degradation(self):
        ...
        print(f"グレースフルデグラデーションテスト - 解析されたブックマーク数: {len(bookmarks)}")  # デバッグ用
